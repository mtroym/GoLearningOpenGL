...
func main() {
	...
	for !window.ShouldClose() {
		drawEBO(vao, ebo, window, program)
	}
}

func makeEbo(indices []int32) uint32 {
	var ebo uint32
	gl.GenBuffers(1, &ebo)
...
func drawEBO(vao uint32, ebo uint32, window *glfw.Window, program uint32) {
...
func initGlfw() *glfw.Window {
	...
	window, err := glfw.CreateWindow(width, height, "0 Triangle", nil, nil)
	if err != nil {
		panic(err)
	}
